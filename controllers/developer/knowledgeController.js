...
exports.listNotes = async (req, res) => {
  try {
    const [notes] = await db.query(
      `SELECT id, title, description, content, is_shared, share_status,
              share_rejection_reason, created_at, updated_at
...
exports.updateNote = async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT id, share_status FROM developer_notes WHERE id = ? AND developer_id = ?',
      [id, devId]
    );
    if (!rows.length) return res.status(404).json({ error: 'Note not found.' });

    const { title, description, content, is_shared } = req.body;
    const currentShareStatus = rows[0].share_status;
    ...
    if (is_shared !== undefined) {
      const wantsToShare = Boolean(is_shared);
      if (wantsToShare) {
        fields.push('is_shared = 1');
        // Only move to pending_review if not already approved or pending
        if (currentShareStatus === 'private' || currentShareStatus === 'rejected') {
          fields.push('share_status = ?');
          vals.push('pending_review');
          fields.push('share_rejection_reason = NULL');
        }
      } else {
        fields.push('is_shared = 0');
        fields.push('share_status = ?');
        vals.push('private');
        fields.push('share_rejection_reason = NULL');
      }
...
// ── Community (admin-approved shared notes from all developers) ───────────────
