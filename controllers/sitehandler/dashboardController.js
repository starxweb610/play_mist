...
exports.getIndex = async (req, res) => {
  let overview = { totalGames: 0, totalUsers: 0, openTickets: 0, pendingReviews: 0 };
  ...
  try {
    const results = await Promise.allSettled([
      ...
      db.query("SELECT COUNT(*) AS c FROM developer_submissions WHERE status = 'pending'"),
    ]);
    ...
    if (results[3].status === 'fulfilled') overview.pendingReviews = results[3].value[0][0].c;
  } catch (_) {}
